...
var jsxer = (_ => x => _ .jsxtion .convert (x)) ({
...
var marked_node = function (attrs, attr_spreads, adoptions) {
...
macro (recursive_pre_transform (calls_ ('h'), function (src, args) {
	var evaler = `(q) => eval (q)`;
	return `____h (${args .map ((x) => JSON .stringify (x, null, 4)) .concat ([evaler]) .join (',')})`
}));
